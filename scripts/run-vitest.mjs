...
/**
 * Runs a Vitest suite and reports it: as this job's verdict, as a section of
 * the shared pull request comment, and as the job summary.
 *
...
async function runSuite(config) {
  ...
  try {
    // Both reporters: the human one keeps the job log readable while the JSON
    // one puts the numbers in a file.
...
async function publish({outcome, results, resultsPath}) {
  const counts = results ? summarize(results) : {passed: 0, failed: 0, total: 0, skipped: 0, files: 0};
  ...
  await setOutput('files', counts.files);
