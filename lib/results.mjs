 ...
 *
 * Vitest writes Jest's aggregate shape, which is why this reads like a Jest
 * reporter — with one difference that matters. `numTotalTestSuites` counts
 * every `describe` as well as every file, so reporting it as "suites" gives a
 * number nobody recognises: a run of 35 files reads as 140 suites. The files
 * are counted instead.
 */
 ...
 * @return {{ passed: number, failed: number, total: number, skipped: number, files: number, seconds: number|null, ok: boolean }} Counts, with `ok` as the runner's own verdict
 */
 ...
 *
 * @param {object} results Aggregated results
 * @return {string} For example `136 passed · 15 files · 1.2s`
 */
...
/**
 * Flattens the per-file results down to the failed assertions.
 *
