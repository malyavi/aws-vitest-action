...
/**
 * Reading the runner's own report. The fixtures are real Vitest output shapes,
 * including the one that matters most and is easiest to forget: a file that
 * failed to load at all, which has no failed assertions to count.
 *
 * Every fixture's `numTotalTestSuites` deliberately disagrees with its file
 * count, because that figure counts every `describe` too and reporting it
 * would give a reader a number they do not recognise.
 */
...
describe('summarize', () => {
  ...
  it('counts files, ignoring the suite total Vitest reports', () => {
    // numTotalTestSuites counts every describe as well as every file: a run of
    // two files reads as eight suites, which is not a number anybody
    // recognises from their own repository.
    const results = load('passed');
    assert.equal(results.numTotalTestSuites, 8);
    assert.equal(summarize(results).files, 2);
  });
...
describe('renderCounts', () => {
  it('reads as a sentence fragment for a passing run', () => {
    assert.equal(renderCounts(load('passed')), '11 passed · 2 files · 1 skipped · 2.4s');
  });

  it('leads with the failures when there are any, since that is what is being read for', () => {
    assert.match(renderCounts(load('failed')), /^2 of 5 tests failed · 1 file/);
  });

  it('says file rather than files for one', () => {
    assert.match(renderCounts(load('failed')), /1 file ·/);
  });

  it('leaves the duration out rather than printing NaN when the timings are absent', () => {
    assert.equal(renderCounts({numPassedTests: 1, testResults: [{name: 'a'}]}), '1 passed · 1 file');
  });
});
...
describe('failedAssertions', () => {
  it('names each failed test and strips the colour codes off its message', () => {
    ...
    assert.equal(failures[0].suite, 'test/app/handlers/orders.test.ts');
    assert.equal(failures[0].name, 'handleOrder marks every answer it gives itself');
    assert.equal(failures[0].message, 'expected undefined to be "X-Request-Id"');
  });

  it('reports a file that never ran as its own entry', () => {
    ...
    assert.match(failures[0].message, /Failed to load/);
  });
  ...
  it('copes with a file entry that carries no path', () => {
...
describe('readResults', () => {
  ...
  it('answers null for a file that is not JSON', async () => {
    assert.equal(await readResults('package.json') === null, false, 'A real JSON file reads.');
    assert.equal(await readResults('README.md'), null);
  });
